...
const Profile = () => {
  ...
  const handleFileUpload = (file) => {
    ...
    /* setting/uploading profile image */
    uploadTask.on(
      "state_changed",
      /* snapshot piece of info from each state change */
      (snapshot) => {
        /* progess process percentage */
        const progress =
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      /* converting what's sent from response into JSON */
  ...
  return (
    <div className="p-3 max-w-lg mx-auto">
      ...
      <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
        ...
        <div className="self-center">
          <img
            ...
            alt="profile"
            className="rounded-full h-24 w-24 object-cover cursor-pointer m-2"
          />
        </div>
        <p className="text-sm self-center">
          {fileUploadError ? (
            <span className="text-red-700 ease-out duration-700 opacity-100">
        ...
        <p className="text-red-500 font-mono text-center">{error && error}</p>
      ...
      {userListings && userListings.length > 0 && (
        <button onClick={handleShowListings} className="text-blue-650 w-full">
          Show Listings
        </button>
      )}
      ...
      {userListings && userListings.length > 0 && (
        <div className="flex flex-col gap-4">
          ...
          {userListings.map((listing) => (
            <div
              key={listing._id}
              className=" border flex justify-between items-center rounded-lg p-3 gap-5"
            >
